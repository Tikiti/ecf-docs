...
#### **ISC Calculation Formulas (Fórmulas de Cálculo ISC) — Developer Reference**
...
The following calculation logic applies to **Fields 107-109** based on tax code ranges defined in **Table I**.
...
#### **MontoTotal & ValorPagar Validation Formulas — Developer Reference**

> [!IMPORTANT]
> These formulas must be correctly implemented for invoice validation to pass.
...
1. **Pagination Consistency** (if Section E exists):
   ```
   MontoTotal = Σ(MontoSubtotalPagina)
   ```
   The total must equal the sum of all page subtotals.

2. **Credit Note Limit** (for Type 34):
   ```
   MontoTotal ≤ MontoTotal_of_NCFModificado
   ```
   Credit notes cannot exceed the original invoice amount.³⁰

3. **Component Non-Negativity**:
   All individual components (MontoGravadoTotal, MontoExento, TotalITBIS, MontoImpuestoAdicional) must be ≥ 0.
...
**With Norma 10-07 Discount:**
```
ValorPagar = MontoTotal - MontoDescuento³¹ - MontoAvancePago ± SaldoAnterior
```

> [!NOTE]
> The ± for `SaldoAnterior` depends on whether the previous balance is positive (customer owes) or negative (customer credit).
